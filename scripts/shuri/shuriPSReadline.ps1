...
function script:script:CSI([string] $content)
{
  "$([char]0x1b)$content"
}
...
Set-PSReadLineOption -PredictionSource HistoryAndPlugin `
  ...
  -ContinuationPrompt "⋯ " `
  -Colors @{ ContinuationPrompt = (script:CSI("[90m")) } `
  -ViModeChangeHandler {
  if ($args[0] -eq 'Command')
  {
    Write-Host -ErrorAction Ignore -NoNewLine (script:CSI("[2 q"))
    $script:PromptChar = "❮"
  } else
  {
    Write-Host -ErrorAction Ignore -NoNewLine (script:CSI("[6 q"))
    $script:PromptChar = "❯"
  }
...
$script:SlowCmd = ""
$script:PromptCharColor = script:CSI("[35m") # magenta
$script:CachedPath = "~"
$global:Jobs = @()
...
$script:OnIdleCallback = {
  Set-PSReadLineOption -ExtraPromptLineCount 1 `
    -ContinuationPrompt "$(script:CSI("[90m"))⋯ $(script:CSI("[0m"))"
  [Microsoft.PowerShell.PSConsoleReadLine]::InvokePrompt()
}
...
function script:UpdateSlowCmd
{
  ...
  if (($lastCmd = Get-History -Count 1))
  {
    ...
    if ($diff -ge ([timespan]::FromSeconds(6)))
    {
      ...
      $script:SlowCmd = "$(script:CSI("[33m"))$script:SlowCmd"
    }
  };
}
...
function global:prompt
{
  if (!$script:InPSReadLine)
  {
    ...
    {
      $script:promptCharColor = script:CSI("[31m") # red
    } else
    {
      $script:promptCharColor = script:CSI("[35m") # magenta
    }
 
    $script:CachedPath = &PromptPath; # try
    # {
  ...
  "$(script:CSI("[34m"))$script:CachedPath$script:SlowCmd`n$script:PromptCharColor$script:PromptChar$(script:CSI("[0m")) "
}
