...
#+begin_src emacs-lisp
  ...
  (defvar lm/font (cond
		 ((string-equal system-type "gnu/linux") "JetBrains Mono Nerd Font-9")
		 ((string-equal system-type "windows-nt") "JetBrains Mono-9")))
...
#+begin_src emacs-lisp
  (if (daemonp)
	...
	(define-key eglot-mode-map (kbd (concat lm/leader "ar")) 'eglot-rename)
	(define-key eglot-mode-map (kbd (concat lm/leader "ao")) 'eglot-code-action-organize-imports)
