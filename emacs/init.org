...
** vterm
vterm is a nice and powerful terminal emulator that works fine on linux but
does not compile under windows. i'll wrote there an eventual solution
#+begin_src emacs-lisp
;;	(use-package vterm)
#+end_src
...
- <Escape> does not need to be pressed three times to behave like <C-g> but this
	has the countereffect of closing other windows when pressed
