...
M.on_attach_functions = {
  ...
  default = function(client, bufnr)
    ...
    vim.keymap.set('n', '<leader>gd', '<cmd>lua vim.lsp.buf.definition()<cr>', { buffer = bufnr, desc = "Definition" })
    ...
    vim.keymap.set('n', '<leader>gl', '<cmd>lua vim.diagnostic.setloclist()<cr>',
