...
This is a quick snippet to configure the emacs sources for linux with pgtk:

#+begin_src shell
  ./configure --prefix=/usr/local --with-pgtk --disable-gc-mark-trace --with-native-compilation=aot --without-compress-install --with-tree-sitter --without-gsettings --without-gconf
#+end_src
